...
class CGP:
    ...
    **kwargs):
        ...
        #table is table of lists of parameters, idx is the depth we are in the table, olds contains stuff appended on earlier recursions
        def read_table(table, idx, olds, combinations):
            temp = olds[:]
            for i, trash in enumerate(table[idx]):
                #copy the current table[idx][i] to a copy of temp for every value of table[idx]
                if(i == 0):
                    temp.append(table[idx][i])
                else:
                    temp[idx]=table[idx][i]
                #use recursion to copy every table[idx+1][i] for every table[idx][i]
                if (idx+1) < len(table):
                    read_table(table, idx+1, temp, combinations)
                else:
                    combinations.append(temp[:])

        
        #Read parameters to sweep

        self.sweep_parameters = kwargs['sweep_parameters']
        table = kwargs['sweep_ranges']
        olds = []
        combinations = []
        read_table(table, 0, olds, combinations)
    ...
    def _rank(self,x,i,k,extra=None,circuit=None):
        ...
        for p in xrange(1,len(f)):
            try:
		#Calculation of fitness.f[p] is tim
    ...
    def step(self):
	#Not needed because only one "generation" is evaluated.
        #self.generation+=1

        #Select chromosomes from pool using weighted probablities
        def sf(pool,weight=1):
            #I assume that the weight is 0-1. Small value gives elitistic choices and 1 takes a random pick form the pool
            r=random.random()**weight
            return random.choice(pool[:1+int(len(pool)*r)])[1]

        #Generate the pool with single netlist but sweeped parameters

        #Randomly generate some circuits

        #R1_values = [1, 10, 100, 130, 160, 150, 170, 330, 470, 540, 680, 1000, 1500, 10000]
        #R2_values = [1, 10, 100, 130, 150, 170, 330, 470, 540, 680, 1000, 1500, 10000]
        poolsize = len(self.combinations)   #len(R1_values)*len(R2_values)
        newpool = [self.chromosome.random_circuit(self.parts_list, self.max_parts, self.sigma, self.inputs, self.outputs, self.special_nodes, self.special_node_prob, extra_value=self.extra_value) for i in xrange(poolsize)]
        #Generate seed circuits
        for i,circuit in enumerate(self.seed_circuits):
            ...
            for i in self.combinations:
                #Copy the seed circuit to every item of the newpool
                newpool[iterator] = deepcopy(newpool[0])
                for j, trash in enumerate(i):
                    newpool[iterator].set_value(i[j], newpool[iterator].index(self.sweep_parameters[j]))
                iterator += 1
        start = time()
        try:
            l = len(newpool)
        ...
        #f is a table that has the result in index[0] and circuits in the subsequent indexes?
        self.saveresults()

       # print self.printpool()
    ...
    def saveresults(self):
        ...
        for f,c in self.pool:
            values.append(f)
            for index, i in enumerate(c.get_values()):
                if(index == 0):
                    R1s.append(i)
                else:
                    R2s.append(i)
        with open('Results.txt', 'w') as f:
            for i, trash in enumerate(values):
                f.write(str(values[i]) + ":" + str(R1s[i]) + ":" + str(R2s[i]) + ":" + '\n')
